...
function Chatbot() {
  ...
  // Mensaje inicial según si viene de un ejercicio
  const initialMessage = ejercicio
    ? {
        role: "bot",
        content: `¿Necesitas ayuda con "${ejercicio.nombre}"? Dime qué necesitas.`,
        timestamp: new Date(),
      }
    : {
        role: "bot",
        content: "Pregúntame lo que quieras",
        timestamp: new Date(),
      };
  ...
  const handleSend = async () => {
    ...
    const now = new Date();
    // 1. Agrega el mensaje del usuario inmediatamente
    setHistory(prev => [
      ...prev,
      { role: "user", content: input, timestamp: now }
    ]);
    setInput(""); // Limpia el input de inmediato
    ...
    try {
      ...
      // 2. Agrega la respuesta del bot cuando llegue
      setHistory(prev => [
        ...prev,
        {
          role: "bot",
      ...
      >
        {history.map((msg, idx) => (
          <div key={idx} style={{
            ...
            {/* Fecha/hora solo para el primer mensaje del grupo */}
